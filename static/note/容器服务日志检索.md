...
原始日志模板(logback.xml): `%d{yyyy-MM-dd HH:mm:ss.SSS}||${center.name}||${app.name}||${ppAgentid}||%highlight(${LOG_LEVEL_PATTERN:-%p})||%boldYellow(%thread)||%boldGreen(%class.%method:%line)||%X{TestFlag}||%X{ReqClientIP}||%X{ReqType}||%X{ReqBizNum}||%X{ReqTimestamp}||%X{ReqDockerIp}||%X{RepServUnitName}||%X{RepServUnitVersion}||%X{RepDockerIp}||%X{Port}||%X{pinpoint-traceid}||%X{pinpoint-pspanid}||%X{pinpoint-spanid}||%X{X-CTG-Request-Id}||%m%n`，单个日志文件限制大小为 100M，超过时自动拆分。
...
public class LogQueryUtil {
    ...
    private List<String> filterLogLines(File file, LogQueryContext logQueryContext) throws IOException {
        ...
        // 利用缓存元数据跳过无需扫描的文件
        LogFileMetadata meta = getOrCreateMetadataV2(file);
    ...
    /**
     * 获取或创建日志文件的元数据（首尾时间戳），优化版本
     */
    private synchronized LogFileMetadata getOrCreateMetadataV2(File file) {
        String filename = file.getName();
        long currentModified = file.lastModified();
